...
class DataLoader {
	...
	void load() {
		...
		fornecedores.forEach( f -> System.out.print(f));
		...
		NotaCompraItem i1_2 = new NotaCompraItem(nc1, p2, new BigDecimal("1000.00"),2);
		NotaCompraItem i1_3 = new NotaCompraItem(nc1, p3, new BigDecimal("500.00"),2);
		i1_1 = notaCompraService.salvar(i1_1);
		i1_2 = notaCompraService.salvar(i1_2);
		i1_3 = notaCompraService.salvar(i1_3);
		
		List<NotaCompraItem> itens = notaCompraService.listaNotaCompraItem();
